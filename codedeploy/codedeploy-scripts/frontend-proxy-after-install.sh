...
## 실행중인 컨테이너 PID 확인
#---------------------------------------------------------------
CURRENT_PID=$(sudo docker container ls -aq --filter name=frontend-proxy)
...
#---------------------------------------------------------------
#docker run -itd --name frontend-proxy -p 80:80 615314566947.dkr.ecr.ap-northeast-2.amazonaws.com/frontend-proxy:latest
docker run -itd --name frontend-proxy --restart unless-stopped --network='host' -p 80:80 615314566947.dkr.ecr.ap-northeast-2.amazonaws.com/frontend-proxy:latest
